...
export async function POST(request: NextRequest) {
  try {
    ...
    // In temp auth mode, register against in-memory store and skip DB
    if (process.env.USE_TEMP_AUTH === '1') {
      const hashedPasswordTemp = await bcrypt.hash(password, 12)
      const created = addMockUser({
        email: email.toLowerCase(),
        name: name.trim(),
        password: hashedPasswordTemp,
        role,
      })

      return NextResponse.json(
        {
          message: "Account created successfully in temp mode. You can now sign in.",
          user: {
            id: created.id,
            email: created.email,
            name: created.name,
    ...
    // Check if user already exists (real DB mode)
