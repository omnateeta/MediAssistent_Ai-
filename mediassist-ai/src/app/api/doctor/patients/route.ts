...
export async function GET(request: NextRequest) {
  try {
    ...
    // Temp mode: derive patients from mock appointments
    if (process.env.USE_TEMP_AUTH === '1') {
      const mockDoctor = ensureMockDoctor(s.user.id, s.user.name, s.user.email)
      const { mockAppointments, mockPatients } = await import("@/lib/mock-store")

      const byThisDoctor = mockAppointments.filter(a => a.doctorId === mockDoctor.id)
      const uniquePatientIds = new Set(byThisDoctor.map(a => a.patientId))
      const patients = Array.from(uniquePatientIds).map(pid => {
        const p = mockPatients.find(mp => mp.id === pid)
        return {
          id: p?.id ?? String(pid),
