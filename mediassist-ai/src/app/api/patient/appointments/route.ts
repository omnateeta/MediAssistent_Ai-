...
import { addMockAppointment, ensureMockPatient } from '@/lib/mock-store'
...
export async function POST(req: Request) {
  try {
    ...
    // In temp mode, bypass DB and use in-memory store
    if (process.env.USE_TEMP_AUTH === '1') {
      const mockPatient = ensureMockPatient(userIdToUse, s.user.name, s.user.email)
      const dateTimeString = `${appointmentDate}T${appointmentTime}:00`
      const appt = addMockAppointment({
        ...
        chiefComplaint,
        patient: { user: { name: s.user.name, email: s.user.email } },
      })
      return NextResponse.json({ appointment: appt, message: 'Appointment created successfully (temp mode)' }, { status: 201 })
    }
...
export async function GET(req: Request) {
  try {
    ...
    if (process.env.USE_TEMP_AUTH === '1') {
      const mockPatient = ensureMockPatient(userIdToUse, s.user.name, s.user.email)
      // Filter mock appointments by patientId
      const { mockAppointments } = await import('@/lib/mock-store')
      const appointments = mockAppointments.filter(a => a.patientId === mockPatient.id)
